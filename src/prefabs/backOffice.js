(() => ({
  ...
  beforeCreate: ({
    ...
    // const [editProperties, setEditProperties] = React.useState(true);
    ...
    const { data } = useModelQuery({
      ...
      onCompleted: response => {
        setProp(response.model.properties.find(p => p.name === 'id'));
        setDetailProperties(response.model.properties);
        console.log({ properties: response.model.properties });
        // setCreateProperties(
        //   response.model.properties.filter(
        //     propertyInput =>
    ...
    const topOrBottomBox = descendants => ({
      name: 'Box',
      options: [
        {
          value: 'space-between',
    ...
    // const createFormDescendantsArray = createPropertiesArray => {
    //   const descendants = createPropertiesArray.map(property => {
      ...
      <>
        ...
        <Footer
          onSave={() => {
            ...
            const detailsInfoCard = tabCard('Record info');
            const detailsRecordInfoProperties = detailProperties.filter(
              propertyInput =>
                propertyInput.label === 'Created at' ||
                propertyInput.label === 'Updated at',
            );
            detailsInfoCard.descendants[1].descendants = makeDetailsArray(
              detailsRecordInfoProperties,
            );

            detailsDataContainer.descendants = [
              ...detailsRecordCard,
              ...detailsInfoCard,
            ];
            ...
            // const createObjectForm = createForm();
            // createObjectForm.options[0].value.modelId = modelId;
            // createObjectForm.options[1].value = modelId;
            // actions[0].events[0].options.assign = createProperties.map(
            //   property => ({
            ...
            //   }),
            // );
            // createObjectForm.descendants = createFormDescendantsArray(
            //   createProperties,
            // );
            // createCard.descendants[1].descendants = [...createObjectForm];
            // createTab.descendants = [topOrBottomBox([]), ...createCard];

            // createObjectForm.options[0].value.modelId = modelId;
            // createObjectForm.options[1].value = modelId;
            // createObjectForm.options[0].value = {
            //   customModelId: null,
            //   actionId: null,
            //   modelId,
            //   variableId: null,
            ...
            // variables[0].options.modelId = modelId;
            // variables[1].options.modelId = modelId;
            // actions[0].events[0].options.modelId = modelId;
            // actions[0].events[0].options.assign = createProperties.map(
            //   property => ({
