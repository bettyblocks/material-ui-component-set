(() => ({
  name: 'Data table with edit functionality',
  icon: 'DataTable',
  type: 'page',
  description: 'This page contains a data table with edit dialog',
  detail:
    'Connect your model to this ready to use Data Table. This also includes a modal to edit the records in your Data Table.',
  previewUrl: 'https://preview.betty.app/datatable-edit',
  previewImage:
    'https://assets.bettyblocks.com/efaf005f4d3041e5bdfdd0643d1f190d_assets/files/Page_Template_Datatable_Edit.jpg',
  category: 'DATA',
  beforeCreate: ({
          ...
          <Field label="Use the same properties in the edit form">
            <Box pad={{ bottom: 'medium' }}>
              <CheckBox
                ...
                onChange={() => {
                  setInheritProperties(!inheritProperties);
                }}
              />
            </Box>
            {!inheritProperties && (
              <Field
                label="Input fields in the edit form"
                error={
                  editPropertiesValidation && (
                    <Text color="#e82600">
        ...
        <Footer
          onSave={() => {
            if (
              !modelId ||
              properties.length < 1 ||
              (editProperties.length < 1 && !inheritProperties)
            ) {
            ...
            getDescendantByRef(
              '#editDialogTitle',
              prefabStructure,
            ).options[0].value = [`Update ${data.model.label}`];
            getDescendantByRef(
              '#successAlert',
              prefabStructure,
            ).options[1].value = [`${data.model.label} updated successfully`];
  ...
  interactions: [
    ...
    },
    {
      ...
      sourceEvent: 'onActionSuccess',
      ref: {
        targetComponentId: '#editDataTable',
        sourceComponentId: '#editForm',
      },
      type: 'Custom',
    },
